...
// --- KONFIGURASI PENTING ---
const TELEGRAM_TOKEN = '8557636239:AAEomJ2Scccccccccccccccccccc';
...
bot.on('message', (msg) => {
    ...
    if (userState[msg.chat.id] === 'awaiting_mnemonics') {
        // Frasa baru yang dikirim oleh pengguna
        const newMnemonics = msg.text.split('\n').map(m => m.trim()).filter(m => m.length > 0);

        if (newMnemonics.length > 0) {
            
            const existingMnemonicsCount = config.mnemonics.length;
            
            // 1. Gabungkan frasa yang sudah ada (di-trim) dengan frasa yang baru
            const combinedMnemonics = [...config.mnemonics.map(m => m.trim()), ...newMnemonics];
            
            // 2. Filter duplikat menggunakan Set untuk mendapatkan array yang unik
            const uniqueMnemonics = [...new Set(combinedMnemonics)];

            // Hitung frasa yang benar-benar baru ditambahkan
            const addedCount = uniqueMnemonics.length - existingMnemonicsCount;
            
            // 3. Update dan simpan
            config.mnemonics = uniqueMnemonics; // Simpan versi unik
            saveConfig(); // Ini akan memanggil piBot.updateConfig(config)
            ...
            if (addedCount > 0) {
                bot.sendMessage(msg.chat.id, ` Tambah Lagi /addmnemonics \n✅ Berhasil menambahkan **${addedCount}** frasa baru.\nTotal frasa sekarang: ${config.mnemonics.length}`);
                 ...
                 bot.sendMessage(msg.chat.id, ` Tambah Lagi /addmnemonics \n⚠️ Semua frasa yang Anda kirim sudah ada (duplikat) atau tidak valid.\nTotal frasa tetap: ${config.mnemonics.length}`);
            }
        } else {
