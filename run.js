...
function updateConfig(newConfig) {
    ...
    if (newConfig.mnemonics && Array.isArray(newConfig.mnemonics)) {
        ...
        // Trim setiap frasa dan gunakan Set untuk memfilter duplikat secara otomatis
        const trimmedMnemonics = newConfig.mnemonics.map(m => m.trim());
        const uniqueMnemonics = [...new Set(trimmedMnemonics)];
        
        // Periksa apakah ada duplikat yang dihapus
        const duplicatesRemoved = trimmedMnemonics.length - uniqueMnemonics.length;
        
        // Update konfigurasi dengan mnemonics yang unik
        currentConfig = { 
            ...newConfig, 
            mnemonics: uniqueMnemonics 
        };
    ...
    console.log(`[Config Updated] Total mnemonics unik: ${currentConfig.mnemonics.length}`);
}
